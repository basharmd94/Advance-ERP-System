...
def create_item_group_api(request):
    ...
    try:
        ...
        item_group_code = item_group_name.lower()
        
        # Check if item group already exists (case-insensitive)
        existing_item_group = Xcodes.objects.filter(
            zid=current_zid,
            xtype='Item Group',
            xcode__iexact=item_group_code
        ).first()
...
def update_item_group_api(request, item_group_code):
    ...
    try:
        ...
        # Convert to lowercase for code
        new_item_group_code_lower = new_item_group_name.lower()
        
        # Check if new item group name already exists (excluding current item group)
        if new_item_group_code_lower != item_group_code.lower():
            existing_item_group = Xcodes.objects.filter(
                zid=current_zid,
                xtype='Item Group',
                xcode__iexact=new_item_group_code_lower
            ).exclude(xcode=item_group_code.lower()).first()
        ...
        Xcodes.objects.filter(
            ...
            xcode=new_item_group_code_lower,
        ...
        return JsonResponse({
            ...
            'item_group': {
                'item_group_code': new_item_group_code_lower,
...
def delete_item_group_api(request, item_group_code):
    ...
    try:
        ...
        # Validate item_group_code is not empty or whitespace
        if not item_group_code or not item_group_code.strip():
            logger.warning(f"Empty or invalid item_group_code provided for deletion: {repr(item_group_code)}")
            return JsonResponse({'status': 'error', 'message': 'Invalid item group code'}, status=400)

        # Normalize code (lowercase)
        normalized_code = item_group_code.strip().lower()

        logger.debug(f"Attempting to delete item group with xcode='{normalized_code}' for ZID={current_zid}")

        # Delete matching record(s) — SAFELY using filter + delete()
        deleted_count, _ = Xcodes.objects.filter(
            zid=current_zid,
            xcode=normalized_code,      # Exact match (already lowercased)
            xtype='Item Group'          # Hardcoded type
        ).delete()

        if deleted_count == 0:
            logger.warning(f"Item group not found for deletion: {normalized_code} for business: {current_zid}")
            return JsonResponse({'status': 'error', 'message': 'Item group not found'}, status=404)

        logger.info(f"Successfully deleted {deleted_count} item group(s): {normalized_code} by user: {request.user.username}")
        return JsonResponse({
            'status': 'success',
